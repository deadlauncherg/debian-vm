...
echo "  5) Root access in google colab"
echo "  0) Exit"
echo
...
# =======================================
# Option 5 → Get root access in Colab + full VM setup
# =======================================
elif [ "$choice" = "5" ]; then
    echo "[INFO] Preparing environment for root access in Google Colab..."

    # Pre-install commands
    sudo apt update
    sudo apt install -y qemu qemu-kvm libvirt-daemon-system libvirt-clients bridge-utils virt-manager qemu-utils
    which qemu-img
    qemu-img --version
